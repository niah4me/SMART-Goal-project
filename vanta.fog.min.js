    ...
    n.VantaBase = class {
        ...
        applyCanvasStyles(e, t={}) {
            Object.assign(e.style, {
                ...
                left: 0,
                background: "",
                height: t.height || "50000vh"
            });
            Object.assign(e.style, t);
            e.style.setProperty("height", t.height || "00000vh", "important"); // Force height
            console.log("Applied styles:", e.style); // Debugging
...
function adjustForScreenSize() {
    ...
    if (width <= 480) {
        // Adjust for mobile
        document.querySelector('#div').style.height = '60vh';
    } else if (width <= 768) {
        // Adjust for tablets
        document.querySelector('#div').style.height = '80vh';
    } else {
        // Default for larger screens
        document.querySelector('#div').style.height = '100vh';
    }
}
...
adjustForScreenSize();
function adjustVantaSettings() {
    const width = window.innerWidth;

    let zoom = 1.9; // Default zoom
    if (width <= 480) {
        zoom = 1.5; // Adjust for mobile
    } else if (width <= 768) {
        zoom = 1.7; // Adjust for tablets
    }

    // Reinitialize Vanta with new settings
    VANTA.FOG({
        el: "#your-background-element",
        zoom: zoom,
        mouseControls: true,
        touchControls: true,
        gyroControls: false
...
// Run on load and resize
window.addEventListener('resize', adjustVantaSettings);
adjustVantaSettings();
function adjustCanvasSize() {
    const canvas = document.querySelector('.vanta-canvas');
    const parent = document.querySelector('#your-background-element');
